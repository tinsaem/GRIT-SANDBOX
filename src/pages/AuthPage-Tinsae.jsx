...
const ROLES = [
  { value: "Student",   label: "Student",   icon: "🎓", desc: "Close skill gaps, get verified, earn tokens" },
  ...
  { value: "Peer",      label: "Peer",      icon: "🤝", desc: "Community-based skill verification" },
];
      ...
      <div style={{ minHeight: "100vh", background: C.ink, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "32px 16px" }}>
          <div style={{ position: "relative", width: "100%", maxWidth: 420, background: C.paper, border: "1px solid " + C.rule, borderTop: "4px solid " + C.green, borderRadius: 8, padding: "36px 32px", boxShadow: "0 20px 60px rgba(0,0,0,.4)", textAlign: "center" }}>
            ...
            <button onClick={handleResendOtp} disabled={loading} style={{ background: "none", border: "none", color: C.teal, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", fontSize: 13, textDecoration: "underline" }}>
              Resend code
            </button>
          </div>
        </div>
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (data?.needsVerification) {
        ...
        } catch { /* account may already have a live code */ }
        setSignupEmail(email);
        setSignupDone(true);
      } else {
                ...
                <div style={{ display: "flex", flexWrap: "nowrap", gap: 6 }}>
                  {ROLES.map((r) => (
                    <button key={r.value} type="button" onClick={() => setRole(r.value)} style={{
                      ...
                      background: role === r.value ? C.gold : "#fff",
                      color: role === r.value ? "#fff" : C.inkSoft,
                      ...
                      transform: role === r.value ? "translateY(-1px)" : "none",
